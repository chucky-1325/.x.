...
-- Adaptado para nexus_operations: sin exports(), sin globales NexusBridgeConfig/
-- NexusBridgeUtils/NexusBridge (usa NexusOperationsConfig.security y un log
-- local), expuesto como tabla NexusOperationsSecurityFallback en vez de
-- funciones globales. Solo cubre rateLimit + timed actions (lo unico que
-- nexus_operations consume de nexus_bridge/server/security.lua) -- no
-- vendoriza el evento securityFlag ni la limpieza en onResourceStop, que son
-- de alcance cross-resource y no aplican aqui.
--
...
NexusOperationsSecurityFallback = {}
...
local function security()
    return NexusOperationsConfig.security or {}
end
...
local function warnFallbackActive()
    ...
    warnedOnce = true
    print('[nexus_operations] AVISO: nexus_bridge no disponible -- usando fallback de seguridad local (rate limit y tokens de accion fisica aislados a este recurso).')
end

function NexusOperationsSecurityFallback.rateLimit(source, bucket)
    warnFallbackActive()
...
function NexusOperationsSecurityFallback.beginTimedAction(source, scope, subject, durationMs)
    warnFallbackActive()
    source = tonumber(source)
...
function NexusOperationsSecurityFallback.consumeTimedAction(source, scope, subject, token)
    warnFallbackActive()
    source = tonumber(source)
    scope = normalizeActionKey(scope, 32)
    subject = normalizeActionKey(subject, 120)
...
function NexusOperationsSecurityFallback.cancelTimedAction(source, scope, token)
    warnFallbackActive()
    source = tonumber(source)
    scope = normalizeActionKey(scope, 32)
