...
-- (canonico) y nexus_operations/server/security_fallback.lua (vendorizado).
--
...
--
-- Uso: lua test/verify_security_fallback.lua   (desde la carpeta nexus_operations)
...
local vendoredEnv = loadInEnv(vendoredPath, makeEnv({
    ...
    AddEventHandler = noop,
    print = noop,
    NexusOperationsConfig = { security = { rateLimits = testRateLimits, timedActions = testTimedActions } },
}))
...
local vendored = vendoredEnv.NexusOperationsSecurityFallback
...
fakeNow = 0
step('beginTimedAction invalido (scope vacio)', function(m) return m.beginTimedAction(2, '', 'op1', 1000) end)

fakeNow = 10000
local canonToken = select(1, canonical.beginTimedAction(2, 'test', 'op1', 1000))
local vendToken = select(1, vendored.beginTimedAction(2, 'test', 'op1', 1000))
...
step('beginTimedAction repetido mientras esta ocupado -> busy', function(m) return m.beginTimedAction(2, 'test', 'op1', 1000) end)

fakeNow = 10100
step('consumeTimedAction demasiado pronto', function(m)
    local token = m == canonical and canonToken or vendToken
    return m.consumeTimedAction(2, 'test', 'op1', token)
end)

fakeNow = 20000
canonToken = select(1, canonical.beginTimedAction(2, 'test', 'op1', 1000))
vendToken = select(1, vendored.beginTimedAction(2, 'test', 'op1', 1000))
...
step('consumeTimedAction token invalido (no-string)', function(m) return m.consumeTimedAction(2, 'test', 'op1', 12345) end)

fakeNow = 40000
canonToken = select(1, canonical.beginTimedAction(3, 'test', 'op2', 1000))
vendToken = select(1, vendored.beginTimedAction(3, 'test', 'op2', 1000))
