...
-- Adaptado para nexus_ems: sin exports(), sin globales NexusBridgeConfig/
-- NexusBridgeUtils/NexusBridge (usa NexusEMSConfig.security y un log local),
-- expuesto como tabla NexusEMSSecurityFallback en vez de funciones globales.
-- Solo cubre rateLimit + timed actions (lo unico que nexus_ems consume de
-- nexus_bridge/server/security.lua) -- no vendoriza el evento securityFlag ni
-- la limpieza en onResourceStop, que son de alcance cross-resource y no
-- aplican aqui.
--
...
local function security()
    return NexusEMSConfig.security or {}
end
...
local function warnFallbackActive()
    ...
    warnedOnce = true
    print('[nexus_ems] AVISO: nexus_bridge no disponible -- usando fallback de seguridad local (rate limit y tokens de accion fisica aislados a este recurso).')
end

function NexusEMSSecurityFallback.rateLimit(source, bucket)
    warnFallbackActive()
...
function NexusEMSSecurityFallback.beginTimedAction(source, scope, subject, durationMs)
    warnFallbackActive()
    source = tonumber(source)
...
function NexusEMSSecurityFallback.consumeTimedAction(source, scope, subject, token)
    warnFallbackActive()
    source = tonumber(source)
    scope = normalizeActionKey(scope, 32)
    subject = normalizeActionKey(subject, 120)
...
function NexusEMSSecurityFallback.cancelTimedAction(source, scope, token)
    warnFallbackActive()
    source = tonumber(source)
    scope = normalizeActionKey(scope, 32)
