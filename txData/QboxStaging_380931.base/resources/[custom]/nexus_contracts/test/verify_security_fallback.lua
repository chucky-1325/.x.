...
-- (canonico) y nexus_contracts/server/security_fallback.lua (vendorizado).
--
...
--
-- Uso: lua test/verify_security_fallback.lua   (desde la carpeta nexus_contracts)
...
-- Reloj compartido y controlable por la prueba.
local fakeNow = 0
...
-- Config de prueba compartida (bucket sintetico 'test_vector', no el 'default'
-- de produccion, para mantener la prueba aislada y con ventanas cortas).
...
local vendoredEnv = loadInEnv(vendoredPath, makeEnv({
    ...
    AddEventHandler = noop,
    print = noop,
    NexusContractsConfig = { security = { rateLimits = testRateLimits, timedActions = testTimedActions } },
}))
...
local vendored = vendoredEnv.NexusContractsSecurityFallback
...
-- Ambos modulos deben ver exactamente el mismo reloj en cada paso: se corre
-- cada vector primero contra el canonico y luego, SIN avanzar el reloj, contra
-- el vendorizado, para que las dos ramas vean el mismo `now`.
...
fakeNow = 1500 -- fuera de la ventana de 1000ms
...
fakeNow = 0
step('beginTimedAction invalido (scope vacio)', function(m) return m.beginTimedAction(2, '', 'lot1', 1000) end)

fakeNow = 10000
local canonToken = select(1, canonical.beginTimedAction(2, 'test', 'lot1', 1000))
local vendToken = select(1, vendored.beginTimedAction(2, 'test', 'lot1', 1000))
...
step('beginTimedAction repetido mientras esta ocupado -> busy', function(m) return m.beginTimedAction(2, 'test', 'lot1', 1000) end)

fakeNow = 10100 -- 100ms despues del begin, duracion 1000, tolerancia 250 -> demasiado pronto
step('consumeTimedAction demasiado pronto', function(m)
    local token = m == canonical and canonToken or vendToken
    return m.consumeTimedAction(2, 'test', 'lot1', token)
end)

-- El consume anterior ya vacio el slot (aunque haya fallado) en ambos modulos;
-- se vuelve a abrir para probar el camino de exito.
fakeNow = 20000
canonToken = select(1, canonical.beginTimedAction(2, 'test', 'lot1', 1000))
vendToken = select(1, vendored.beginTimedAction(2, 'test', 'lot1', 1000))

fakeNow = 20800 -- 800ms >= duracion(1000) - tolerancia(250) = 750, y < expiresAt (20000+1000+2000)
...
canonToken = select(1, canonical.beginTimedAction(2, 'test', 'lot1', 1000))
vendToken = select(1, vendored.beginTimedAction(2, 'test', 'lot1', 1000))
fakeNow = 30000 + 1000 + 2000 + 5000 -- muy por encima de expiresAt
step('consumeTimedAction expirado', function(m)
    local token = m == canonical and canonToken or vendToken
    return m.consumeTimedAction(2, 'test', 'lot1', token)
end)

step('consumeTimedAction token invalido (no-string)', function(m) return m.consumeTimedAction(2, 'test', 'lot1', 12345) end)

fakeNow = 40000
canonToken = select(1, canonical.beginTimedAction(3, 'test', 'lot2', 1000))
vendToken = select(1, vendored.beginTimedAction(3, 'test', 'lot2', 1000))
